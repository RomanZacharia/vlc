...
package-macosx-release:
	...
	-cp "$(macos_destdir)$(pkglibexecdir)/vlc-cache-gen" $(top_builddir)/vlc-$(VERSION)-release/ 2>/dev/null || cp "$(top_builddir)/bin/vlc-cache-gen" $(top_builddir)/vlc-$(VERSION)-release/ 2>/dev/null || true
	-if test -f $(top_builddir)/vlc-$(VERSION)-release/vlc-cache-gen; then \
